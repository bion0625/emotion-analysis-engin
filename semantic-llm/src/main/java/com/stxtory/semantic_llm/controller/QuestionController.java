...
public class QuestionController {
    ...
    /**
     * 사용자 글을 받아 정신분석학적 관점의 질문을 생성하는 API 엔드포인트.
     * HTTP POST 요청의 본문(body)에 사용자 글을 담아 보냅니다.
     *
     * @param userText HTTP 요청 본문에 담긴 사용자 글 (RequestBody 어노테이션으로 매핑)
    ...
    public static class WebConfig implements WebMvcConfigurer {
        ...
        public void addCorsMappings(CorsRegistry registry) {
            registry.addMapping("/**") // 모든 경로에 대해 CORS 허용 규칙을 적용합니다.
                    // 허용할 Origin(출처)을 지정합니다. 개발 환경에서는 localhost:3000(React/Vue 등)이 일반적입니다.
