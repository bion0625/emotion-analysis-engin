...
public class OllamaChatConfig {
	...
	/**
	 * Spring AI의 ChatClient 빈을 생성하여 등록합니다. `spring-ai-starter-model-ollama` 의존성이 `ChatModel`
	 * (OllamaChatModel 구현체) 빈을 자동으로 생성하여 Spring 컨텍스트에 등록합니다. 우리는 그 자동 구성된 `ChatModel` 빈을 여기에 주입받아
	 * `ChatClient`를 만듭니다.
	 *
	 * @param chatModel
	 *            Spring AI 스타터가 자동 구성한 ChatModel 빈 (OllamaChatModel)
