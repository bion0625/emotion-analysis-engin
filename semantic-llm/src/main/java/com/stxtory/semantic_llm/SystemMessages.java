...
public class SystemMessages {
	...
	public static final String STOCK = """
			분석 기준:
			- 급등하는 가격 상승이 반드시 좋은 신호는 아니며, 거래량이 과도하게 증가하면 단기 과열로 판단할 수 있어.
			- 이상적인 종목은 가격이 서서히 상승하고, 거래량도 점진적으로 증가하는 우상향 추세를 보이는 것이야.
			- 반대로 가격이 하락하면서 거래량이 급증하는 경우는 반등 가능성이 있는 스윙 구간일 수 있어.

			요청 사항:
			1. 위 기준을 기반으로 주목할 종목 1~2개를 추천해줘.
			2. 각 종목을 추천한 이유를 가격과 거래량 변화 중심으로 설명해줘.
			3. 가능하다면 향후 단기/중기 시나리오 예측도 포함해줘.
